...
* Create a new directory hierarchy: <code>levels/''my-levels''</code>
* Copy your levels (*.stl files) into your that folder.
* If you made a [[worldmap]], you have to put it
** '''0.1:''' in <code>levels/worldmaps/</code> as <code>''my-levels''.stwm</code>, or
** '''0.3:''' in your <code>levels/''my-levels''/</code> as <code>worldmap.stwm</code>.
* To make it an addon, create a <code>''my-levels''.nfo</code> text file in the top level of your directory hierarchy and fill it with the following information. See the [[#.nfo files]] section below for details.
 (supertux-addoninfo
...
* So that it shows up in the "Contrib Levels" menu, you need to create an <code>levels/''my-levels''/info</code> file. You really only need this:
 (supertux-world
...
* Use a ZIP utility to package up your directory into <code>my-levels.zip</code>.
...
* You've made a package. Now log on to [[Contact#IRC|IRC]] and let us know about your addition!&nbsp;:)
...
* As the file <code>''my-levels''/''my-levels''.nfo</code> within your add-on.
* As a text-file retrieved from an URL.

==== Valid keys ====
:''(As of [[Template:Revision|6398]])''
; kind
...
* If it's advertised as a 0.3.x addon, it probably is. To install this, just unpack it in your supertux data directory, or better yet put it in your .supertux2 directory. (See [[Console]] for the location; it's the directory with the config file in it) Starting with Subversion [[Template:revision|5458]], you can also put the archives into the data folder without bothering to unpack them.
