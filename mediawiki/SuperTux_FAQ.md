...
We plan to release the Linux version on 2038-11-30. Not sure if we
will do a GNU Hurd version by then, too. The schedule is a bit
sketchy right now.
...
OpenGL was and is the current default renderer. However, 0.3.1 added in support for multiple renderers. The command line option <code>--renderer&nbsp;sdl</code> will use the SDL renderer. Note that this is usually slower and does not display some images/levels correctly.
...
* [[Contributing|Create game content]] in the form of sounds, music, [[Translation|translations]], [[Contributing#Artists|images]], and [[Contributing#Level designers|levels]] - see [[Contributing]] for details
* Write [[Contributing#Programmers/Code|code]] for new features
* Proofread and improve the program's and the levels' translation (See [[Translation]])
* Work on the story and the settings (all around the wiki, mostly in [[:Category:Design]])
...
This fix is currently in the 0.1.x branch and [http://lists.lethargik.org/pipermail/supertux-devel-lethargik.org/2010-March/002941.html will be released in 0.1.4] as soon as someone can find the time to actually make the release.
...
Versions ''0.3.0'' and ''0.3.1'' contained the [[Forest|Forest world]]. In an attempt to get development up to speed after a period of idleness, the focus was narrowed. Development of the Forest world has been moved to [[Milestone 3]]. The ''0.3.*'' and eventually the ''0.4.*'' versions implement the [[Milestone 2 Design Document]], i.e. a game without the Forest world.
