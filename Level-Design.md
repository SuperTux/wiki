...
might not be playable on slower machines anymore.
...
Use small clues to hint at hidden secrets (e.g. a single coin otherwise unreachable). Should be visible if player is paying attention
but not obvious.
...
* Don't allow dead ends or impossible levels; either the player dies, or there's still a way to solve it. (Allowing the player to
  suicide from a trap works as well) Test your level thoroughly to find possible paths you missed before.
* Avoid too hard or too easy levels. Too easy levels get boring very quickly while too hard ones can be frustrating. Levels you
  built often appear a lot easier for yourself, so be sure to let other people test them.
...
* Encourage exploration away from the central path with non essential bonuses.
...
* Try and show off new features, but only ones that are in the latest stable version.
* Make sure that there is an easy level that encourages learning any new skill safely before challenging players
